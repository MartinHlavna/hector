CLOSE_WORD_PREFIX = "close_word_"
CLOSE_WORD_RANGE_PREFIX = "_range_"
FREQUENT_WORD_PREFIX = "frequent_word_"
CLOSE_WORD_TAG_NAME = "close_word"
FREQUENT_WORD_TAG_NAME = "frequent_word"
GRAMMAR_ERROR_TAG_NAME = "grammar_error"
MULTIPLE_PUNCTUATION_TAG_NAME = "multiple_punctuation"
TRAILING_SPACES_TAG_NAME = "trailing_spaces"
COMPUTER_QUOTE_MARKS_TAG_NAME = "computer_quote_mark"
DANGLING_QUOTE_MARK_TAG_NAME = "dangling_quote_mark"
SHOULD_USE_LOWER_QUOTE_MARK_TAG_NAME = "should_use_lower_quote_mark"
SHOULD_USE_UPPER_QUOTE_MARK_TAG_NAME = "should_use_upper_quote_mark"
MULTIPLE_SPACES_TAG_NAME = "multiple_spaces"
PARAGRAPH_TAG_NAME = "paragraph"
LONG_SENTENCE_TAG_NAME_MID = "long_sentence_mid"
LONG_SENTENCE_TAG_NAME_HIGH = "long_sentence_high"
SEARCH_RESULT_TAG_NAME = "search_result"
CURRENT_SEARCH_RESULT_TAG_NAME = "current_search_result"
BOLD_TAG_NAME = "bold"
ITALIC_TAG_NAME = "italic"
FORMATTING_TAGS = {
    PARAGRAPH_TAG_NAME,
    SEARCH_RESULT_TAG_NAME,
    CURRENT_SEARCH_RESULT_TAG_NAME,
    BOLD_TAG_NAME,
    ITALIC_TAG_NAME
}
