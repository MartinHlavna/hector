...
class ProjectSelectorWindow:
    def __init__(self, r):
        ...
        tk.Button(left_side_panel, text="Nový projekt", command=self.open_new_project_form, width=15, cursor="hand2",
                  background=PRIMARY_COLOR, foreground=PANEL_TEXT_COLOR, relief=tk.FLAT, borderwidth=0).pack(
            pady=5
        )
        tk.Button(left_side_panel, text="Otvoriť projekt", command=self.open_project_from_file, width=15,
                  cursor="hand2",
                  background=PRIMARY_COLOR, foreground=PANEL_TEXT_COLOR, relief=tk.FLAT, borderwidth=0).pack(
            pady=5
        )
        ...
        self.browse_image = GuiUtils.fa_image(FA_SOLID, TEXT_EDITOR_BG, PANEL_TEXT_COLOR, FontAwesomeIcons.folder, 19,
                                              padding=4)
        project_location_browse_btn = tk.Button(project_location_frame, command=self.select_new_project_location,
                                                width=19,
                                                cursor="hand2",
        ...
        # SAVE BUTTON
        buttons = [
            tk.Button(button_frame, text="Vytvoriť projekt", command=self.create_project, width=15, cursor="hand2",
                      background=PRIMARY_COLOR, foreground=PANEL_TEXT_COLOR, relief=tk.FLAT, borderwidth=0),
            tk.Button(button_frame, text="Zrušiť", command=self.close_new_project_form, width=15, cursor="hand2",
                      background=PRIMARY_COLOR, foreground=PANEL_TEXT_COLOR, relief=tk.FLAT, borderwidth=0)
        ]
